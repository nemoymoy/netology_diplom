        ...
        if {'items'}.issubset(request.data):
            for item in request.data["items"].split(','):
                try:
                    ...
                    return JsonResponse({'Status': False, 'Error': 'Недопустимый тип аргумента (items).'},
                                        status=status.HTTP_400_BAD_REQUEST)
                except ObjectDoesNotExist:
                    return JsonResponse({'Status': False, 'Error': f'Нет контакта с ID{item}'},
                                        status=status.HTTP_400_BAD_REQUEST)
            return JsonResponse({'Status': True}, status=status.HTTP_200_OK)
        return JsonResponse({'Status': False, 'Error': 'Не указаны все необходимые аргументы'},
                            status=status.HTTP_400_BAD_REQUEST)
...
class ShopStatus(APIView):
    ...
    def post(request, *args, **kwargs):
        ...
        if my_status:
            try:
                Shop.objects.filter(user_id=request.user.id).update(status=literal_eval(my_status))
                return JsonResponse({'Status': True})
        ...
        if {'id', 'contact'}.issubset(request.data):
            if request.data['id'].isdigit():
                try:
                    ...
                    return JsonResponse({'Status': False, 'Errors': f'Неверно указаны аргументы {error}'},
                                        status=status.HTTP_400_BAD_REQUEST)
                else:
                    if is_updated:
    ...
    def post(request, *args, **kwargs):
        ...
        if items:
            try:
                ...
                JsonResponse({'Status': False, 'Errors': f'Недопустимый формат запроса {e}'})
            else:
    ...
    def put(request, *args, **kwargs):
        ...
        if items_sting:
            try:
                ...
                return JsonResponse({'Status': True, 'Создано объектов': objects_updated})
        return JsonResponse({'Status': False, 'Errors': 'Не указаны все необходимые аргументы'},
                            status=status.HTTP_400_BAD_REQUEST)
    @staticmethod
    def delete(request, *args, **kwargs):
        ...
        if items:
            ...
            if objects_deleted:
                ...
                return JsonResponse({'Status': True, 'Удалено объектов': deleted_count}, status=status.HTTP_200_OK)
        return JsonResponse({'Status': False, 'Error': 'Не указаны все необходимые аргументы'},
                            status=status.HTTP_400_BAD_REQUEST)
    ...
    def post(request, *args, **kwargs):
        ...
        if url:
            try:
                ...
                return JsonResponse({'Status': False, 'Errors': f'Ошибка целостности: {e}'})
            return JsonResponse({'Status': True}, status=status.HTTP_200_OK)
        return JsonResponse({'Status': False, 'Errors': 'Не указаны все необходимые аргументы'},
                        status=status.HTTP_400_BAD_REQUEST)
